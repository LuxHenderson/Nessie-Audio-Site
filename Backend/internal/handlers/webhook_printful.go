...
func (h *Handler) handlePrintfulOrderFailed(payload PrintfulWebhookPayload) {
	...
	go func() {
		...
		contentHTML := fmt.Sprintf(`<p style="font-size:16px;">A Printful order has failed and may require manual intervention.</p>%s%s%s`,
			email.InfoBox("Order Details",
				email.DetailRow("Order ID:", fmt.Sprintf("#%s", orderID))+
					email.DetailRow("Printful Order ID:", fmt.Sprintf("%d", payload.Order.ID))+
					email.DetailRow("Customer Email:", customerEmail)),
			email.NoteBox("<strong>Action Required</strong><br>Please check the Printful dashboard and contact the customer if a refund is needed.", true),
			email.CTAButton("Open Printful Dashboard", "https://www.printful.com/dashboard/orders"),
		)
		htmlBody := email.EmailLayout("Order Failed", "&#9888;", contentHTML, true)
